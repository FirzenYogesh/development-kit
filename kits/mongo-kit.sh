...
setExecutables() {
    ln -sfn "$WORKSPACE/current/bin/mongo" "$DEVELOPMENT_KIT_EXECUTABLES/mongo"
    ln -sfn "$WORKSPACE/current/bin/mongod" "$DEVELOPMENT_KIT_EXECUTABLES/mongod"
    ln -sfn "$WORKSPACE/current/bin/mongos" "$DEVELOPMENT_KIT_EXECUTABLES/mongos"
}
...
if [[ $MODE == "install" ]]; then
    ...
    switchVersion
    if ! command -v mongod &> /dev/null || ! command -v mongo &> /dev/null || ! command -v mongos &> /dev/null ; then
        setExecutables
    fi
    setEnv
    ...
    if [[ $OS == "linux" ]]; then
        systemctl --user stop mongod.service
        systemctl --user disable mongod.service
    elif [[ $MODE == "macos" ]]; then
        launchctl stop development-kit.mongodb
